...
export const useGraphStore = create<GraphState>((set, get) => ({
    ...
    generateGraphTimeline: async (numNodes: number, edgeProbability: number) => {
        ...
        try {
            const response = await axios.post(`https://algorithmvisualizer-zxqj.onrender.com/api/v1/graphs/generate`, {
                num_nodes: numNodes,
    ...
    runGraphAlgorithm: async () => {
        ...
        try {
            ...
            const response = await axios.post(`https://algorithmvisualizer-zxqj.onrender.com${endpoint}`, {
